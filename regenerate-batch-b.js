...
function createMetaDescription(w1, w2, slug) {
  ...
  // Build with the understanding that we need 140-155 chars minimum
  const base = `Compare ${name1} vs ${name2}: ${size1} vs ${size2} case, ${type1} vs ${type2}. Prices: ${formatPrice(price1)} vs ${formatPrice(price2)}.`;
  
  let desc = base;
  
  // Pad to 140+ chars if needed
  if (desc.length < 140) {
    desc = base.replace('.', ' Find the perfect watch for your wrist today.');
  }
  
  // Ensure we don't exceed 155
  if (desc.length > 155) {
    // Truncate gracefully
    desc = desc.substring(0, 152) + '...';
  }
  
  // Final check: if still under 140, add more
  if (desc.length < 140) {
    desc = `Compare ${name1} vs ${name2}: ${size1} case vs ${size2} case, ${type1} movement vs ${type2} movement. Price: ${formatPrice(price1)} vs ${formatPrice(price2)}. Full details inside.`;
  }
...
batchB.forEach(pair => {
  ...
  if (w1 && w2) {
    const entry = {
      ...
      meta_description: createMetaDescription(w1, w2, pair.slug),
...
// Merge with existing, skip if already present
const allEntries = [...rawOverrides];
...
console.log('✓ New Batch B entries created:', newEntries.length);
console.log('✓ Skipped (already present):', skipped);
console.log('✓ Total entries now:', allEntries.length);
console.log('✓ Meta descriptions in range (140-155):', inRange, '/', newEntries.length);
...
[0, 15, 30].forEach(idx => {
  if (newEntries[idx]) {
    ...
    console.log(`\n${e.slug}`);
    console.log(`  Meta (${e.meta_description.length} chars): "${e.meta_description}"`);
  }
});
