  ...
  return (
    <div className="relative">
      ...
      <button
        ...
        className="w-full bg-[#0f172a] border border-[#334155] rounded-xl px-4 py-3 text-left flex items-center justify-between hover:border-[#d4a853]/40 transition-colors focus:outline-none focus:border-[#d4a853]"
      >
        {selected ? (
          <div>
            <p className="text-xs text-[#d4a853] font-semibold">{selected.brand}</p>
            <p className="text-white font-medium">{selected.name}</p>
          </div>
        ) : (
          <span className="text-slate-500">Select a watch...</span>
        )}
        ...
        <div className="absolute z-10 w-full mt-1 bg-[#1e293b] border border-[#334155] rounded-xl shadow-2xl overflow-hidden">
          <div className="p-2 border-b border-[#334155]">
            <input
              ...
              autoFocus
              className="w-full bg-[#0f172a] rounded-lg px-3 py-2 text-sm text-white placeholder-slate-500 focus:outline-none"
            />
          </div>
          <div className="max-h-64 overflow-y-auto">
            {filtered.map((w) => (
              <button
                ...
                className={`w-full px-4 py-2.5 text-left hover:bg-[#334155] transition-colors flex items-center justify-between ${value === w.slug ? 'bg-[#d4a853]/10' : ''}`}
              >
                <div>
                  <p className="text-xs text-[#d4a853]">{w.brand}</p>
                  <p className="text-sm text-white">{w.name}</p>
                </div>
                <span className="text-xs text-slate-500">{formatPrice(w.price_new_usd)}</span>
              </button>
            ))}
          </div>
        </div>
  ...
  return (
    <div>
      <div className="text-center mb-6">
        <div className="bg-gradient-to-br from-[#1e293b] to-[#0f172a] rounded-xl border border-[#334155] aspect-square max-w-48 mx-auto flex items-center justify-center mb-4 overflow-hidden">
          {watch.image ? (
            <img
              src={watch.image}
            ...
            <svg className="w-12 h-12 text-[#334155]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <circle cx="12" cy="12" r="9" strokeWidth="1" />
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="1" d="M12 7v5l3 3" />
            </svg>
          )}
        </div>
        <p className="text-xs text-[#d4a853] font-bold uppercase tracking-wider mb-1">{watch.brand}</p>
        <h2 className="text-xl font-bold text-white mb-1">{watch.name}</h2>
        <p className="text-slate-500 text-xs mb-3">Ref. {watch.reference}</p>
        {overall && (
          <div className="text-2xl font-bold text-[#d4a853]">{overall.toFixed(1)}<span className="text-sm text-slate-400 font-normal">/5</span></div>
        )}
      </div>
...
export default function CompareClient({ watches, initialA, initialB }: Props) {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-white mb-2">Compare Watches</h1>
        <p className="text-slate-400">Select two watches to see them side-by-side</p>
      </div>
      ...
      {watchA && watchB && (
        <>
          ...
          <div className="card overflow-hidden mb-8">
            <div className="grid grid-cols-3 bg-[#1e293b] px-4 py-3 border-b border-[#334155]">
              <div className="text-slate-400 text-sm font-medium">Specification</div>
              <div className="text-white font-semibold text-sm">{watchA.brand} {watchA.name}</div>
              <div className="text-white font-semibold text-sm">{watchB.brand} {watchB.name}</div>
            </div>
            {SPEC_ROWS.map((row, i) => {
              ...
              return (
                <div key={row.label} className={`grid grid-cols-3 px-4 py-3 text-sm border-b border-[#334155] last:border-0 ${i % 2 === 0 ? 'bg-[#0f172a]' : 'bg-[#1e293b]'}`}>
                  <div className="text-slate-400">{row.label}</div>
                  <div className={`capitalize ${differ ? 'text-[#d4a853] font-medium' : 'text-white'}`}>{valA}</div>
                  <div className={`capitalize ${differ ? 'text-[#d4a853] font-medium' : 'text-white'}`}>{valB}</div>
                </div>
              )
            })}
          </div>
      ...
      {/* Popular comparisons */}
      <div className="mt-14">
        <h2 className="text-xl font-bold text-white mb-5">Popular Comparisons</h2>
