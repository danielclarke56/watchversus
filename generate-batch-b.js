...
function createMetaDescription(w1, w2) {
  ...
  if (price1 && price2) {
    ...
    desc = `Compare ${name1} and ${name2}: ${size1} vs ${size2}, ${formatPrice(price1)} vs ${formatPrice(price2)}. Detailed specs & verdict.`;
  } else {
    desc = `${name1} vs ${name2} detailed comparison. Case size ${size1} vs ${size2}, ${type1} vs ${type2} movements. Find the best fit.`;
  }
  
  // Ensure 140-155 chars
  if (desc.length < 140) {
    if (price1 && price2) {
      desc = `Compare ${name1} (${size1}) and ${name2} (${size2}): ${type1} vs ${type2} movements. ${formatPrice(price1)} vs ${formatPrice(price2)}. Which watch wins?`;
    }
  }
